...
async function savePixTransaction(pixData) {
  ...
  writeDebugToFile(filename, 'DEBUG: pixData desestruturado com sucesso.');

  writeDebugToFile(filename, `DEBUG: savePixTransaction - filename: ${filename}`);
  writeDebugToFile(filename, `DEBUG: savePixTransaction - valor: ${valor}`);
  writeDebugToFile(filename, `DEBUG: savePixTransaction - destinatario: ${destinatario}`);
  writeDebugToFile(filename, `DEBUG: savePixTransaction - pagador: ${pagador}`);
  ...
  writeDebugToFile(filename, `DEBUG: tableName determinado como: ${tableName}`);
  ...
  writeDebugToFile(filename, `DEBUG: About to execute SQL query for ${tableName}.`);
  try {
...
/**
 * Atualiza uma transação PIX existente na tabela pix_transactions_no_process.
 * @param {number} id - O ID da transação a ser atualizada.
 * @param {Object} pixData - Os dados da transação PIX a serem atualizados.
 * @returns {Promise<Object>} - Resultado da atualização.
 */
async function updatePixTransactionNoProcess(id, pixData) {
  return new Promise((resolve, reject) => {
    const {
      valor,
      destinatario,
    ...
    const sql = `
      UPDATE pix_transactions_no_process SET
        valor = ?,
        destinatario = ?,
        chave_pix = ?,
        data_transferencia = ?,
    ...
    query(sql, values)
      .then(result => {
        console.log(`✅ Transação não processada com ID ${id} atualizada no banco. Linhas afetadas: ${result.affectedRows}`);
        resolve({ success: true, affectedRows: result.affectedRows });
      })
      .catch(err => {
        console.error('Erro ao atualizar transação não processada no banco:', err);
        reject(err);
      });
  });
}
...
module.exports = {
  ...
  getFilenamesFromNoProcessTable,
  deleteFileFromNoProcessTable,
  updatePixTransactionNoProcess // Adicionar a nova função aqui
};

/**
 * Deleta um arquivo da tabela pix_transactions_no_process pelo nome do arquivo.
 * @param {string} filename - O nome do arquivo a ser deletado.
 * @returns {Promise<Object>} - Resultado da exclusão (affectedRows).
 */
async function deleteFileFromNoProcessTable(filename) {
  console.log(`DEBUG: Attempting to delete ${filename} from pix_transactions_no_process.`);
  return new Promise((resolve, reject) => {
    const sql = 'DELETE FROM pix_transactions_no_process WHERE filename = ?';
    query(sql, [filename])
      .then(result => {
        console.log(`DEBUG: Deleted ${result.affectedRows} rows for ${filename} from pix_transactions_no_process.`);
        resolve({ success: true, affectedRows: result.affectedRows });
      })
      .catch(err => {
        console.error(`ERROR: Erro ao deletar ${filename} de pix_transactions_no_process:`, err);
        reject(err);
      });
  });
}
...
/**
 * Busca apenas os nomes dos arquivos da tabela pix_transactions_no_process.
 * @returns {Promise<Array>} - Lista de nomes de arquivos não processados.
 */
async function getFilenamesFromNoProcessTable() {
  console.log('DEBUG: getFilenamesFromNoProcessTable called.');
  return new Promise((resolve, reject) => {
    const sql = 'SELECT filename FROM pix_transactions_no_process ORDER BY processed_at DESC';
    console.log(`DEBUG: Executing SQL: ${sql}`);
    query(sql)
      .then(results => {
        console.log(`DEBUG: Found ${results.length} filenames in pix_transactions_no_process.`);
        resolve(results.map(row => row.filename));
      })
      .catch(err => {
        console.error('ERROR: Erro ao buscar nomes de arquivos não processados:', err);
        reject(err);
      });
  });
}
